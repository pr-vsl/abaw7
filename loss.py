...
def CCCLoss(y_hat, y, num_classes=2):
...
def MSELoss(y_hat, y, num_classes=2):
    mse_loss = torch.mean(torch.square(y_hat - y), dim=0)
...
def SigmoidFocalLoss(y_hat, y, num_classes=12, alpha=0.25, gamma=2):
    y_hat_fl = torch.reshape(y_hat, (-1, num_classes))
    y_fl = torch.reshape(y, (-1, num_classes)) * 1.0
...
def CEFocalLoss(y_hat, y, num_classes=8, label_smoothing=0., alpha=0.25, gamma=2.,
